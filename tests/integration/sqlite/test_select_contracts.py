...
def test_for_update_rejects_non_lockable_query_shapes_and_unknown_tables() -> None:
    ...
    arm = select(users.id).from_(users)
    with pytest.raises(ValueError, match="UNION, INTERSECT, or EXCEPT"):
        compile_postgres(arm.union(arm.for_update()))
    with pytest.raises(ValueError, match="UNION, INTERSECT, or EXCEPT"):
        compile_postgres(arm.union(arm).union(arm.for_update()))
