...
from tests.integration.postgres.support import jobs, ledger, slots
...
async def conflict_database(postgres_admin: asyncpg.Connection) -> PostgresDatabase:
    ...
    await postgres_admin.execute("""create table relq_integration_ledger (
    ...
    await postgres_admin.execute(
        "create unique index relq_integration_ledger_key on relq_integration_ledger "
        "(tenant, queue, dedupe_key, epoch, day) "
...
def _ledger_upsert(state: str, revision: int):
    """A five-column arbiter carrying both predicates at once."""
    return (
        insert_into(ledger)
        .values(
            tenant=1,
            queue="emails",
            dedupe_key="welcome:7",
            epoch=7,
            day=datetime.date(2026, 1, 1),
            state=state,
            revision=revision,
        )
        .on_conflict(ledger.tenant, ledger.queue, ledger.dedupe_key, ledger.epoch, ledger.day)
        .where(ledger.dedupe_key.is_not_null() & ledger.state.in_(DEDUPE_STATES))
        .do_update(revision=excluded(ledger.revision))
        .where(ledger.revision.lt(excluded(ledger.revision)))
    )


async def test_a_wide_composite_arbiter_honours_both_predicates(
    conflict_database: PostgresDatabase,
) -> None:
    """Five target columns, past the four the old overload ladder could spell.

    The arity fix has to hold up where it matters: a composite arbiter that
    still infers a partial unique index and still gates its update.
    """
    database = conflict_database
    compiled = compile_postgres(_ledger_upsert("pending", 1))
    assert (
        'on conflict ("tenant", "queue", "dedupe_key", "epoch", "day") where '
        '(("relq_integration_ledger"."dedupe_key" is not null) and '
        "(\"relq_integration_ledger\".\"state\" in (E'pending', E'leased')))" in compiled.sql
    )

    assert await database.execute(_ledger_upsert("pending", 1)) == 1
    revision = select(ledger.revision).from_(ledger)
    assert await database.fetch_all(revision) == [(1,)]

    # Same five-column key, so the arbiter matches; the action predicate is
    # true, so the row moves.
    assert await database.execute(_ledger_upsert("pending", 5)) == 1
    assert await database.fetch_all(revision) == [(5,)]

    # The arbiter still matches, but 3 < 5 is false, so the stored row is
    # left alone -- and no second row appears.
    assert await database.execute(_ledger_upsert("pending", 3)) == 0
    assert await database.fetch_all(revision) == [(5,)]

    # Dropping one of the five columns out of the key makes it a different
    # row entirely, which is what proves all five reached the arbiter.
    distinct = (
        insert_into(ledger)
        .values(
            tenant=1,
    ...
    assert await database.execute(distinct) == 1
    assert await database.fetch_all(
        select(ledger.epoch, ledger.revision).from_(ledger).order_by(ledger.epoch.asc())
    ) == [(7, 5), (8, 1)]
