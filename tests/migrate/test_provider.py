...
def test_provider_orders_numbered_files_and_ignores_non_sql_files(tmp_path: Path) -> None:
    ...
    _write(tmp_path, "local.txt", "not a migration")
...
def test_provider_rejects_misnamed_sql_files(tmp_path: Path) -> None:
    _write(tmp_path, "0001_create_users.sql", "create table users (id integer);")
    _write(tmp_path, "0002_Add_Index.sql", "create index users_id on users (id);")
