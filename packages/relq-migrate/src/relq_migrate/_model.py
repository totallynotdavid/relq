...
def validate_identifier(identifier: str, *, kind: str, max_bytes: int | None = None) -> str:
    ...
    if max_bytes is not None and len(identifier.encode("utf-8")) > max_bytes:
        raise ValueError(f"{kind} identifier exceeds the {max_bytes}-byte limit")
    return identifier
...
def is_sql_empty(sql: str) -> bool:
    """Return whether SQL contains only whitespace, semicolons, and comments."""
    index = 0
    while index < len(sql):
